...
cout<<"89hhhh"<<endl;
	...
	while(nh.ok())
    {
		...
		//multi object
		vector<std::pair<cv::Point,long long>> center_l,center_r;//center and size of connected component of l & r img.
		...
		std::sort(center_l.begin(),center_l.end(),[](const std::pair<cv::Point,long long>&a,const std::pair<cv::Point,long long>&b){return a.second>b.second;});//sort the l & r connected component by size to match them for disparity.
		...
		for(int i=0;i<center_r.size();i++)
			circle(img2r,center_r[i].first,5,Scalar(0,0,0),3,8,0);//draw multi object
