...
Camera::Camera() {
	...
	calib_P = cvMat(3, 4, CV_64F, calib_P_data);
	calib_K_ = cvMat(3, 3, CV_64F, calib_K_data_);
	calib_rotMatrix_ = cvMat(3, 3, CV_64F, calib_rotMatrix_data_);
	calib_transVec_ = cvMat(4, 1, CV_64F, calib_transVec_data_);

	memset(calib_K_data,0,sizeof(double)*3*3);
	memset(calib_D_data,0,sizeof(double)*4);
	memset(calib_P_data,0,sizeof(double)*3*4);
	memset(calib_K_data_,0,sizeof(double)*3*3);
	memset(calib_rotMatrix_data_,0,sizeof(double)*3*3);
	memset(calib_transVec_data_,0,sizeof(double)*4);
...
void Camera::SetCameraInfo(const sensor_msgs::CameraInfo &camInfo)
{
    ...
    cvmSet(&calib_P, 0, 0, cam_info_.P[0]);
    cvmSet(&calib_P, 0, 1, cam_info_.P[1]);
    cvmSet(&calib_P, 0, 2, cam_info_.P[2]);
    cvmSet(&calib_P, 0, 3, cam_info_.P[3]);
    cvmSet(&calib_P, 1, 0, cam_info_.P[4]);
    cvmSet(&calib_P, 1, 1, cam_info_.P[5]);
    cvmSet(&calib_P, 1, 2, cam_info_.P[6]);
    cvmSet(&calib_P, 1, 3, cam_info_.P[7]);
    cvmSet(&calib_P, 2, 0, cam_info_.P[8]);
    cvmSet(&calib_P, 2, 1, cam_info_.P[9]);
    cvmSet(&calib_P, 2, 2, cam_info_.P[10]);
    cvmSet(&calib_P, 2, 3, cam_info_.P[11]);

    cvDecomposeProjectionMatrix(&calib_P, &calib_K_, &calib_rotMatrix_, &calib_transVec_);
}
					...
					Pose *pose)
{
	...
	cvFindExtrinsicCameraParams2(object_points, image_points, &calib_K_, NULL, &ext_rodriques_mat, &ext_translate_mat);

    pose->SetRodriques(&ext_rodriques_mat);
	pose->SetTranslation(&ext_translate_mat);

  //Pose cam_to_world;
  //cam_to_world.Rotation::SetMatrix(calib_rotMatrix_);
  //cam_to_world.SetTranslation()
	cvReleaseMat(&object_points);
	cvReleaseMat(&image_points);
}
					...
					CvMat *rodriques, CvMat *tra)
{
  ...
  cvZero(tra);
	...
	cvFindExtrinsicCameraParams2(&world_mat, &image_mat, &calib_K_, NULL, rodriques, tra);
...
bool Camera::CalcExteriorOrientation(const CvMat* object_points, CvMat* image_points, CvMat *rodriques, CvMat *tra) {
	...
	cvFindExtrinsicCameraParams2(object_points, image_points, &calib_K_, NULL, rodriques, tra);
	return true;
}
...
void Camera::ProjectPoints(vector<CvPoint3D64f>& pw, Pose *pose, vector<CvPoint2D64f>& pi) const {
	...
	cvProjectPoints2(object_points, &ext_rodriques_mat, &ext_translate_mat, &calib_K_, NULL, image_points);  
	for (size_t i=0; i<pw.size(); i++) {
				   ...
				   const CvMat* translation_vector, CvMat* image_points) const
{
	...
	cvProjectPoints2(object_points, rotation_vector, translation_vector, &calib_K_, NULL, image_points);  
}
