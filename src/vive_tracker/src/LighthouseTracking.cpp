...
//user defined pose output function
void LighthouseTracking::getPoseMatrix(int deviceClass, int deviceId,double pose[])
{
    //HMD
    ...
    if(deviceClass == TrackedDeviceClass_Controller )
    {
        ...
        VRControllerState_t controllerState;
        HmdQuaternion_t rot;
        ControllerData* pC = &(controllers[deviceId]);
        if (pC->deviceId < 0 ||
                !vr_pointer->IsTrackedDeviceConnected(pC->deviceId)||
                pC->hand </*=  Allow printing coordinates for invalid hand? Yes.*/ 0)
            return;
    ...
    //generic tracker
    if(deviceClass == TrackedDeviceClass_GenericTracker )
    {
        TrackedDevicePose_t trackedDevicePose;
        VRControllerState_t controllerState;
        HmdQuaternion_t rot;
        TrackerData* pT = &(trackers[deviceId]);
